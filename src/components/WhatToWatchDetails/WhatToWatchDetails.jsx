...
import "./WhatToWatchDetails.css";
import Spinner from "../Spinner/Spinner"; 

export default function WhatToWatchDetails() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        const filtered = response.data.filter((movie) => movie.category === "toWatch");
        setMovies(filtered);
      } catch (err) {
  ...
  return (
    <Container className="py-5 mt-5">
      <div className="mb-4">
        <h1 className="text-light">Top Picks</h1>
      ...
      {movies.length === 0 ? (
        <div className="text-center py-5">
          ...
          <p className="text-light mt-3">No recommendations available right now.</p>
        </div>
      ) : (
