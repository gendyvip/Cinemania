...
import "./FanFavoritesDetails.css";
import Spinner from "../Spinner/Spinner";

export default function FanFavoritesDetails() {
  ...
  useEffect(() => {
    const fetchFanFavorites = async () => {
      try {
        ...
        setMovies(response.data.filter((movie) => movie.category === "favorite"));
      } catch (err) {
        console.error("Error fetching fan favorites:", err);
  ...
  return (
    <Container className="py-5 mt-5">
      <div className="mb-4">
        <h1 className="text-light">Fan Favorites</h1>
        <p className="text-secondary">This week's top TV and movies</p>
      </div>
      
      {movies.length === 0 ? (
        <div className="text-center py-5">
          <i className="bi bi-emoji-frown text-warning" style={{ fontSize: "2rem" }}></i>
          <p className="text-light mt-3">No fan favorites available at the moment.</p>
        </div>
      ) : (
